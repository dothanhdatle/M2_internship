...
def reconstruct_one(model, var, env, device):
    ...
    with torch.no_grad():
        ...
        z_v = model.z_v_dict[var].to(device)
        z_e = model.z_e_dict[env].to(device)
        z = torch.cat((z_v, z_e), dim = -1).to(device)

        model.pxz_distribution.param = model.decode(z)
...
def reconstruct_all(model, nirs_data, device):
    """
...
def wavelength_mse(miss_recon, ground_truth):
    wl = ground_truth.iloc[:,2:].columns
    wl = [float(s.replace(',', '.')) for s in wl]
    mse_wl_list = []
    for i in range(len(miss_recon)):
        variete = miss_recon.iloc[i,0]
        env = miss_recon.iloc[i,1]
        ...
        mse_wl_list.append((nirs_rec-nirs_gt)**2)
    
    mse_wl_list = np.array(mse_wl_list)
    mse_wl = np.mean(mse_wl_list)
    
    return wl, mse_wl

def rec_wl(wl, miss_recon, ground_truth):
    rec_val = []
    gt_val = []
    for i in range(len(miss_recon)):
        variete = miss_recon.iloc[i,0]
        env = miss_recon.iloc[i,1]
        rec_val.append(miss_recon.loc[i,wl])
        nirs_gt = ground_truth[(ground_truth.GenoID == variete) & (ground_truth.Environnement == env)]
        if len(nirs_gt) == 0:
            #print(f'No NIRS groundtruth available for the {variete} in the environment {env}')
            continue
        else:
            gt_val.append(nirs_gt.loc[0,wl])
    return rec_val, gt_val
